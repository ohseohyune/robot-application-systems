...
int main(int argc, char *argv[]) {
    ...
    double sum_sq_err_x[2] = {0.0, 0.0};
    double max_abs_err_x[2] = {0.0, 0.0};
    ...
    for (int64_t i = 0; i < total_loops; ++i) {
        ...
        cartesian_ref(sim_t, xr, yr, dxr, dyr, ddxr, ddyr);

        double dx[2];

        // dx = J(q) * dq
        dx[0] = J_raw[0][0] * dq[0] + J_raw[0][1] * dq[1];
        dx[1] = J_raw[1][0] * dq[0] + J_raw[1][1] * dq[1];
        ...
        for (int k = 0; k < 2; ++k) {
            ...
            double ex = shm->x_ref[k] - shm->x[k];

            double edx = shm->dx_ref[k] - shm->dx[k];

            shm->err_x[k] = ex;
            shm->err_dx[k] = edx;

            sum_sq_err_x[k] += ex * ex;

            if (fabs(ex) > max_abs_err_x[k]) max_abs_err_x[k] = fabs(ex);

            shm->rms_err_x[k] = sqrt(sum_sq_err_x[k] / (double)(i + 1));
            shm->max_abs_err_x[k] = max_abs_err_x[k];
        }
